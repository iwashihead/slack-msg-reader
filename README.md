...
#### 特定の人の発言だけを高速収集する（検索ベース、実験的）

```bash
slack collect --user haru           # 表示名 or 既知のユーザーID or 生のSlack ID (U0123ABCなど)
```

`--user` を指定すると、全チャンネルを1つずつスクロールする代わりに、Slack自身の検索
（`from:<@ユーザーID>`、新しい順）でその人の発言を横断的に探します。他の全オプション
（`--kind`/`--channel-id`/`--full-history`/`--threads` など）はこのモードでは無視されます。

- 圧倒的に高速（該当チャンネルだけを開く通常収集と違い、検索結果を舐めるだけ）
- その代わり精度は落ちます: リアクションは取得されず、スレッド返信かどうかの情報も
  失われます（検索結果のスニペットにその情報が含まれないため）
- Slack自前の検索UIをブラウザ操作で駆使する都合上、通常収集より不安定です
  （検索欄の状態が壊れて結果が0件になる、など）。安定した収集が必要な場合は
  代わりに `slack report --user <名前>` を使ってください
...
- ユーザーはSlackの実際のユーザーID（DOM上の `data-message-sender` 属性、例: `U0123ABC`、
  Slackbotなどのアプリは `USLACKBOT` 固定）をキーに管理しています。表示名が変わっても
  同一人物として扱われます。**この変更より前に収集したデータ**は表示名を正規化したキーで
  保存されているため、同一人物が新旧で別ユーザー扱いのまま残ります。統一したい場合は
  `data/slack_archive.db` を削除して `--full-history` で再収集してください。
